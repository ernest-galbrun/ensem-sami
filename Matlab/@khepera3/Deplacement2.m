function Deplacement2( k3,t )
% Deplacement prend en entrée le tableau préalablement créé par la fonction "Initialisation",
% Puis, une fois la fonction lancée, elle tourne jusqu'à l'arrivée de tous les kheperas à leur destination.
% Pour lancer la fonction, tous les khepera doivent être connecté et en position.
...
while (a)
    ...
    for j= 1: nb_robots     % Boucle qui se répètera pour chacun des robots successivement
        ...
        if(arret(j)==false)
            t{j,4}=Autorisation(X,t{j,4}); % modifie l'autorisation d'un robot en fonction de la position des autres.
        end
...
function [priorite] = Autorisation(X,priorite)  % X est le vecteur des valeurs lues par les lecteurs. Il est mis à jour à chaque boucle du programme Deplacement.   

for x=1: length(priorite(:,1))
    
    if priorite(x,3) ~= 0              % Pour éviter d'appeler X(0), qui existe pas.
        
        if X(priorite(x,3)) == priorite(x,2)      % test du passage d'un robot au lecteur n°priorite(x,3); priorite(x,2)=numéro robot
            priorite(x,2)=0;                      % autorisation de passer en modifiant le tableau p.
            priorite(x,3)=0;
        end
    end
end
