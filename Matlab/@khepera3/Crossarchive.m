function Crossarchive( k3,id, dr, agr, vel ) % dr= direction du robot avec 0=tout droit; 1=à droite; 2=à gauche.
global B;
...
if (tourn(id)==false && V(10)>3900 && V(11)>3900) % Test d'arrivée a une intersection
...
if dr==1
    if (tourn(id) && A>B(id)-1.4)      % Test actif quand le robot tourne
        calllib('khepera3clib', 'SetSpeed',k3.id,50,-1);
    end
    
    if (tourn(id) && A<=B(id)-1.4)         % Test de fin de virage
