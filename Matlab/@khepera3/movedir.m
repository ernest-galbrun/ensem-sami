function [t,D,O,V]=movedir(O,D,A,dx,dy,t) %fonction qui fait tourner à gauche ou à droite (dépend de l'orientation)
...
if (k>0)
    ...
    t=[t;1;0];D(1)=D(1)+dx;D(2)=D(2)+dy;O=-1i*O;%V=[V;D];disp('droite');disp('position :');disp(D);
end
end
