...
if (D(2)==A(2))                   %cas ou D et A sont à  même ordonnée (dy==0) 
    if (rem(A(2),2)==0)           % si on peut se déplacer directement selon x (c-a-d y est pair) on va directement à  A, sinon on tourne, avance selon x, retourne.
        for i=1:(abs(A(1)-D(1))/2)
            disp('tout droit');
            D(1)=D(1)+2*dx;disp('position :');disp(D);
        end
        ...
        if (rem(D(2)+dx,4)==0) % détermine où est l'intérieur du noeud (si yD + dx est divisible par 4 l'intérieur est en (xD +dx,yD-dx) sinon (xD +dx,yD+dx))
            [u D O]=move7b(O,D,[D(1)+dx,D(2)-dx]);t=[t;u];[u D O]=move7b(O,D,[A(1)-dx,D(2)]);t=[t;u];[u D O]=move7b(O,D,A);t=[t;u];
        else
            [u D O]=move7b(O,D,D+dx);t=[t;u];[u D O]=move7b(O,D,[A(1)-dx,D(2)]);t=[t;u];[u D O]=move7b(O,D,A);t=[t;u];
        end
    end
...
elseif(D(1)==A(1))                   %cas ou D et A sont à  même abscisse (dx==0)
    ...
    if (rem(A(1),2)==0)           % si on peut se déplacer directement selon y (c-a-d x est pair) on va directement à  A, sinon on tourne, avance selon x, retourne.
        for i=1:(abs(D(2)-A(2))/2);
                disp('tout droit');
                D(2)=D(2)+2*dy;disp('position :');disp(D);
        end
        
    else
        if (rem(D(1)+dy,4)==0)
            [u D O]=move7b(O,D,[D(1)-dy,D(2)+dy]);t=[t;u];[u D O]=move7b(O,D,[D(1),A(2)-dy]);t=[t;u];[u D O]=move7b(O,D,A);t=[t;u];
        else
            [u D O]=move7b(O,D,D+dy);t=[t;u];[u D O]=move7b(O,D,[D(1),A(2)-dy]);t=[t;u];[u D O]=move7b(O,D,A);t=[t;u];
        end
    end
    ...
    if (rem(D(1),2)==0)
        if (rem(A(1),2)==0)
            if (rem(A(2)+dy,4)==0)
                [u D O]=move7b(O,D,[D(1),A(2)-2*dy]);t=[t;u];
                [t D O]=movedir(O,D,A,dx,dy,t);
                [u D O]=move7b(O,D,[A(1)-dx,D(2)]);t=[t;u];
                [t D O]=movedir(O,D,A,dx,dy,t);
            else
                [u D O]=move7b(O,D,[D(1),A(2)]);t=[t;u];[u D O]=move7b(O,D,A);t=[t;u];
            end
            
        else
            [u D O]=move7b(O,D,[D(1),A(2)-dy]);t=[t;u];
            [t D O]=movedir(O,D,A,dx,dy,t);
            [u D O]=move7b(O,D,A);t=[t;u];
        end
    else
        if (rem(A(2),2)==0)
            if (rem(A(1)+dx,4)==0)
                [u D O]=move7b(O,D,[A(1)-2*dx,D(2)]);t=[t;u];
                [t D O]=movedir(O,D,A,dx,dy,t);
                [u D O]=move7b(O,D,[D(1),A(2)-dy]);t=[t;u];
                [t D O]=movedir(O,D,A,dx,dy,t);
            else
                [u D O]=move7b(O,D,[A(1),D(2)]);t=[t;u];[u D O]=move7b(O,D,A);t=[t;u];
            end
        else
            [u D O]=move7b(O,D,[A(1)-dx,D(2)]);t=[t;u];
            [t D O]=movedir(O,D,A,dx,dy,t);
