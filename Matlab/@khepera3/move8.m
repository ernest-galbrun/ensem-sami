...
V=D;
if(A==D)//cas où on est arrivé pour clore la récursion : on ne fait rien (sinon : récursion)
else
...
if (D(2)==A(2))                   //cas ou D et A sont à même ordonnée (dy==0) 
    if (rem(A(2),2)==0)           // si on peut se déplacer directement selon x (c-a-d y est pair) on va directement à A, sinon on tourne, avance selon x, retourne.
        for i=1:(abs(A(1)-D(1))/2)
            t=[t;0;0];disp('tout droit');
            D(1)=D(1)+2*dx;V=[V;D];disp('position :');disp(D);
        end
        
    else
        if (rem(D(2)+dx,4)==0) // détermine où est l'intérieur du noeud (si yD + dx est divisible par 4 l'intérieur est en (xD +dx,yD-dx) sinon (xD +dx,yD+dx))
            [u D O W]=move8(O,D,[D(1)+dx,D(2)-dx]);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,[A(1)-dx,D(2)]);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,A);t=[t;u];V=[V;W(2:length(W)/2,:)];
        else
            [u D O W]=move8(O,D,D+dx);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,[A(1)-dx,D(2)]);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,A);t=[t;u];V=[V;W(2:length(W)/2,:)];
        end
    end
...
elseif(D(1)==A(1))                   //cas ou D et A sont à même abscisse (dx==0)
    ...
    if (rem(A(1),2)==0)           // si on peut se déplacer directement selon y (c-a-d x est pair) on va directement à A, sinon on tourne, avance selon x, retourne.
        ...
        if (rem(D(1)+dy,4)==0)
            [u D O W]=move8(O,D,[D(1)-dy,D(2)+dy]);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,[D(1),A(2)-dy]);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,A);t=[t;u];V=[V;W(2:length(W)/2,:)];
        else
            [u D O W]=move8(O,D,D+dy);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,[D(1),A(2)-dy]);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,A);t=[t;u];V=[V;W(2:length(W)/2,:)];
        end
    end
    ...
    if (rem(D(1),2)==0)
        if (rem(A(1),2)==0)
            if (rem(A(2)+dy,4)==0)
                [u D O W]=move8(O,D,[D(1),A(2)-2*dy]);t=[t;u];V=[V;W(2:length(W)/2,:)];
                [t D O V]=movedir(O,D,A,V,dx,dy,t);
                [u D O W]=move8(O,D,[A(1)-dx,D(2)]);t=[t;u];V=[V;W(2:length(W)/2,:)];
                [t D O V]=movedir(O,D,A,V,dx,dy,t);
            else
                [u D O W]=move8(O,D,[D(1),A(2)]);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,A);t=[t;u];V=[V;W(2:length(W)/2,:)];
            end
            
        else
            [u D O W]=move8(O,D,[D(1),A(2)-dy]);t=[t;u];V=[V;W(2:length(W)/2,:)];
            [t D O V]=movedir(O,D,A,V,dx,dy,t);
            [u D O W]=move8(O,D,A);t=[t;u];V=[V;W(2:length(W)/2,:)];
        end
    else
        if (rem(A(2),2)==0)
            if (rem(A(1)+dx,4)==0)
                [u D O W]=move8(O,D,[A(1)-2*dx,D(2)]);t=[t;u];V=[V;W(2:length(W)/2,:)];
                [t D O V]=movedir(O,D,A,V,dx,dy,t);
                [u D O W]=move8(O,D,[D(1),A(2)-dy]);t=[t;u];V=[V;W(2:length(W)/2,:)];
                [t D O V]=movedir(O,D,A,V,dx,dy,t);
            else
                [u D O W]=move8(O,D,[A(1),D(2)]);t=[t;u];V=[V;W(2:length(W)/2,:)];[u D O W]=move8(O,D,A);t=[t;u];V=[V;W(2:length(W)/2,:)];
            end
        else
            [u D O W]=move8(O,D,[A(1)-dx,D(2)]);t=[t;u];V=[V;W(2:length(W)/2,:)];
            [t D O V]=movedir(O,D,A,V,dx,dy,t);
